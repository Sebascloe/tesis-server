...
from services.provincia import create_provincia, get_all_provincias, get_one_provincia, update_one_provincia
from typing import List

router = APIRouter()

@router.post("/provincia", response_model=ProvinciaResponse)
def create_provincia_archivo_endpoint(provincia:ProvinciaCreate, session: Session = Depends(get_session)):
...
def patch_provincia_endpoint(provincia_id: int, provincia: ProvinciaCreate, session: Session = Depends(get_session)):
